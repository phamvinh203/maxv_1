...
<grid table="ct60gl" code="stt_rec" order="stt_rec" type="Detail" freezeColumns="3" xmlns="urn:schemas-maxv-com:data-grid">
  ...
  <fields>
    ...
    <field name="ten_tk_vt" hidden="true">
      <header v="" e=""></header>
    </field>

    <field name="ma_kh_thue" width="100">
      <header v="Mã NCC (phần thuế)" e="Supplier Code"></header>
    ...
    <field name="ma_nk" width="80">
	  ...
	  <items style="AutoComplete" controller="VoucherBook" key="status = '1' and ma_nk in (select ma_nk from v20dmctnk where ma_ct = 'HM3' and (lstuser = '' or charindex(rtrim(@@userName) + ',', replace(lstuser, ' ', '') + ',') > 0))" check="1=1" information="ma_nk$v20dmnk.ten_nk%l"/>
    </field>
  </fields>
  ...
  <queries>
    <query event="Loading">
      <text>
        <![CDATA[<encrypted>x9J6x+fGL3k0+FImW3uVsnzaru8GaqGBLRrC2RVlT0kZD2iuJcDfBtmc7LB62+4eZTTu4AOeprkNBHx44S9M6fe7gzJMYqbxwPEG5vxPM80TXqFihefyCyk2t/4TXkPeBoKawRYz7Cl6v/x4nRs6PA==</encrypted>]]>
      </text>
    </query>
  </queries>
