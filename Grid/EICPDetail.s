...
<grid table="ct60gl" code="stt_rec" order="stt_rec" type="Detail" freezeColumns="3" xmlns="urn:schemas-maxv-com:data-grid">
  ...
  <fields>
    ...
    <field name="ma_nk" width="80">
	  ...
	  <items style="AutoComplete" controller="VoucherBook" key="status = '1' and ma_nk in (select ma_nk from v20dmctnk where ma_ct = 'BN1' and (lstuser = '' or charindex(rtrim(@@userName) + ',', replace(lstuser, ' ', '') + ',') > 0))" check="1=1" information="ma_nk$v20dmnk.ten_nk%l"/>
    </field>
  </fields>
  ...
  <queries>
    <query event="Loading">
      <text>
        <![CDATA[<encrypted>x9J6x+fGL3k0+FImW3uVsnzaru8GaqGBLRrC2RVlT0kZD2iuJcDfBtmc7LB62+4eZTTu4AOeprkNBHx44S9M6UPe8mXL55TJnII0DFM+C4aWGDcuy/BKVJtaT3+0DnpV5vGL43xrhUHpft6aZ3V8Jw==</encrypted>]]>
      </text>
    </query>
  </queries>
